...
function readSkill(dir, name) {
  ...
  if (!front.description)
    fail(`${name}: SKILL.md frontmatter is missing "description"`);
  if (front.name && front.name !== name) {
    fail(
      `${name}: SKILL.md frontmatter name is "${front.name}" but the directory is "${name}"`,
    );
  }
...
const config = safeJsonParse(fs.readFileSync(CONFIG, "utf8"), {});
if (!config || typeof config !== "object") fail(`failed to parse ${CONFIG}`);
...
for (const name of listDirs(pluginsDir)) {
  ...
  if (isGenerated) {
    if (!generatedNames.has(name)) {
      if (PRUNE)
        fs.rmSync(path.join(pluginsDir, name), {
          recursive: true,
          force: true,
        });
      else
        fail(
          `plugins/${name} is generated but has no root skill dir; delete it or run with --prune`,
        );
    }
    continue;
  }
  ...
  if (!fs.existsSync(manifest)) {
    fail(
      `plugins/${name} has neither a generated skill symlink nor a plugin.json`,
    );
    continue;
  }
  const parsed = safeJsonParse(fs.readFileSync(manifest, "utf8"));
  if (!parsed) {
    fail(`failed to parse JSON manifest at ${manifest}`);
    continue;
  }
  vendored.push({ name, ...parsed });
}

if (errors.length === 0 && rootSkills.length === 0)
  fail("no skill directories found at the repo root");

// --- Plan ------------------------------------------------------------------

const plan = new Map(); // repo-relative path -> { kind, content }
const addFile = (rel, content) => plan.set(rel, { kind: "file", content });
const addLink = (rel, target) =>
  plan.set(rel, { kind: "symlink", content: target });
...
for (const skill of rootSkills) {
  ...
  addLink(
    path.join("plugins", skill.name, "skills", skill.name),
...
for (const category of config.categories) {
  ...
  for (const name of category.skills) {
    if (!blurbs.has(name)) {
      fail(
        `skills.json category "${category.title}" lists "${name}", which is not a skill or plugin`,
      );
      continue;
    }
...
for (const name of blurbs.keys()) {
  if (!categorized.has(name))
    fail(`"${name}" is not listed under any category in skills.json`);
}
...
if (begin === -1 || end === -1) {
  ...
  addFile(
    "README.md",
    readme.slice(0, begin) + body + readme.slice(end + README_END.length),
  );
}
...
if (CHECK) {
  ...
  console.log(
    `sync is up to date (${rootSkills.length} skills, ${vendored.length} vendored plugins)`,
  );
}
...
const targets = [
  ROOT,
  ...entries.map((e) => path.join(ROOT, "plugins", e.name)),
];
let validated = 0;
for (const target of targets) {
  try {
    ...
    console.error(
      `validation failed for ${path.relative(ROOT, target) || "."}:`,
    );
